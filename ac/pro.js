...
function _bsaPRO() {
    ...
    function flushWrite(which, data, incremental) {
        ...
        if (!writer && data.length)
            writer = which.parentNode.insertBefore(document.createElement('div'), which);
        if (writer) {
            writer.id = id;
    ...
    function findScripts(t) {
        for (var i = 0; i < t.childNodes.length; i++)
            t.childNodes[i].nodeName.toLowerCase() == 'script' && t.childNodes[i].className != 'ignoreme' && (!t.childNodes[i].type || t.childNodes[i].type.indexOf('script') != -1) ? gotScript(t.childNodes[i]) : findScripts(t.childNodes[i])
    }
    var htmls = {};
    ...
    function setInnerHtmlAndExec(el, html, incremental) {
        ...
        if (isValidHtml(htmls[el.id]) || !incremental) {
            ...
            while (temp.childNodes.length > 0)
                el.appendChild(temp.childNodes[0])
        }
        if (!incremental)
            findScripts(el)
    }
    ...
    for (var zones = [], zonesegments = {}, cl = function(cl) {
        ...
        for (var k = 0; k < bs[i].attributes.length; k++) {
            if (bs[i].attributes[k].name.substring(0, 12) == 'data-segment') {
                ...
                for (var valsi = 0; valsi < vals.length; valsi++)
                    seg.push([segkey, vals[valsi]])
            }
        }
    ...
    if (zones.length) {
        ...
        if (force)
            url += '&forcebanner=' + force[1];
        else if (window['_bsap_forcebanner'])
            url += '&forcebanner=' + window['_bsap_forcebanner'];
        if (ignore)
            url += '&ignorebanner=' + ignore[1];
        else if (window['_bsap_ignorebanner'])
            url += '&ignorebanner=' + window['_bsap_ignorebanner'];
        if (ignoretarg)
            url += '&ignoretargeting=yes';
        else if (window['_bsap_ignoretargeting'])
            url += '&ignorebanner=yes';
        if (maxp)
            url += '&maxpriority=' + maxp[1];
        else if (window['_bsap_maxpriority'])
            url += '&maxpriority=' + window['_bsap_maxpriority'];
        if (nostats)
            url += '&ignore=yes';
        if (preview)
            segments.push([preview[1], [['SEGMENT', 'previewonly']]]);
        for (k = 0; k < segments.length; k++) {
            seg = [];
            for (var j = 0; j < segments[k][1].length; j++)
                seg.push(segments[k][1][j][0] + ':' + segments[k][1][j][1]);
            zonesegments[segments[k][0]] = seg.join(';');
            if (seg.length)
                url += '&segment_' + segments[k][0] + '=' + seg.join(';')
        }
    }
    ...
    if (day.length || life.length) {
        ...
        if (freqcap.length)
            url += '&freqcap=' + encodeURIComponent(freqcap.join(';'))
    }
    c.src = url;
