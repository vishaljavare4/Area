var _bsap = {
    ...
    deploy: function() {
        for (var zi = 0, wi, z, a, b, db; zi < this.jz.length && (z = this.jz[zi]) && (wi = 1); zi++)
            while (++wi < 10 && (db = this.dz[z.id]) && (a = db.pop()) && (b = document.getElementById(a)))
    ...
    zone: function(b, z, bsa) {
        function doc_write(write) {
            if (!bsa.writes[bsa.writescript.id])
                bsa.writes[bsa.writescript.id] = '';
        ...
        function stepQueue(which) {
            if (bsa.scriptqueue.length > 0 && bsa.scriptqueue[0].which == which && !bsa.scriptqueue[0].run)
        ...
        bsa.filter(z.filterby, function(f) {
            ...
            if (z.premium) {
                if (!bsa.prem[z.premium])
                    bsa.prem[z.premium] = 1;
            ...
            if (tf == 0) {
                ...
                for (i = 0,
                j = 0; i < c.length && (a = bsa.getads(c[i], 1)[0]); i++,
                (Math.random() * 100 < (z.model == 1 && z.nads > c.length ? (100 - c[i - 1].per) : empty)) || (j++,
                o += (a.rawframe ? ('<iframe width="' + w + '" height="' + h + '" id="ad_' + a.id + '_frame" src="' + a.rawframe + '" frameborder="0" class="bsap_adframe" scrolling="no"><\/iframe>') : (a.flash && bsa.hasFlash() ? ('<object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" width="' + z.width + '" height="' + z.height + '" id="bsa_flash"><param name="movie" value="' + a.flash + '"/><param name="menu" value="false"/><param name="quality" value="high"/><param name="wmode" value="opaque"/><param name="allowScriptAccess" value="always"/><param name="FlashVars" value="clickTag=' + encodeURIComponent(bsa.tracker('click.go', a.id, z.id, '', '', '&link=' + a.link.replace('[timestamp]', ts).replace(/&amp;/g, '&'))) + '"/><embed src="' + a.flash + '" width="' + z.width + '" height="' + z.height + '" quality="high" wmode="opaque" menu="false" allowScriptAccess="always" swliveconnect="true" flashvars="clickTag=' + encodeURIComponent(bsa.tracker('click.go', a.id, z.id, '', '', '&link=' + a.link.replace('[timestamp]', ts).replace(/&amp;/g, '&'))) + '" type="application/x-shockwave-flash"></embed></object>') : ('<a ' + bsa.link(a.id, z.id, a.link.replace('[timestamp]', ts)) + ' class="ad' + j + ' ' + (j % 2 === 0 ? 'even' : 'odd') + '" title="' + a.alt + '" id="bsa_' + a.id + '" target="_blank"><img src="' + a.img.replace('[timestamp]', ts) + '" width="' + z.width + '" height="' + z.height + '" alt="' + a.alt + '"\/><\/a>'))),
                t += a.id + ';')) {}
                ...
                for (i = j; !backfilled && i < z.nads && ah && (i < (ra ? z.nads : (j + 1))); i++,
                o += (bsa.callback(z.id, i - 1, z.nads) || '<a href="https:\/\/www.buysellads.com\/buy\/detail\/' + z.siteid + '\/zone\/' + z.id + '?utm_source=site_' + z.siteid + '&utm_medium=website&utm_campaign=adhere&utm_content=zone_' + z.id + '" title="Advertise Here" class="adhere ad' + i + ' ' + (i % 2 === 0 ? 'even' : 'odd') + '" target="_blank">Advertise Here<\/a>')) {}
                if (z.model == 1 && ah && (j == z.nads || backfilled))
                    o += '<div class="bsap_adhere' + (w == 728 ? '2' : '') + '"><a href="https:\/\/www.buysellads.com\/buy\/detail\/' + z.siteid + '\/zone\/' + z.id + '?utm_source=site_' + z.siteid + '&utm_medium=website&utm_campaign=cpmadhere&utm_content=zone_' + z.id + '" target="_blank">advertise here<\/a><\/div>'
            } else if (tf == 1) {
                ...
                for (i = 0,
                a; i < c.length && (a = bsa.getads(c[i], 1)[0]); i++,
                (j++,
                o += '<div class="bsa_it_ad ad' + i + ' ' + (i % 2 === 0 ? 'even' : 'odd') + '" id="bsa_' + a.id + '"><a ' + bsa.link(a.id, z.id, a.link.replace('[timestamp]', ts)) + ' target="_blank"><span class="bsa_it_i"><img src="' + a.img.replace('[timestamp]', ts) + '" width="' + z.width + '" height="' + z.height + '" alt="' + a.alt + '" \/><\/span><\/a><a ' + bsa.link(a.id, z.id, a.link) + ' target="_blank"><span class="bsa_it_t">' + a.alt + '<\/span><span class="bsa_it_d">' + a.text + '<\/span><\/a><div style="clear:both"><\/div><\/div>',
                t += a.id + ';')) {}
                if (z.cpmbackfill && j == 0) {
                    for (var r = Math.random(), bn = 0; bn < z.cpmbackfill.length && z.cpmbackfill[bn] && z.cpmbackfill[bn][0] < r; r -= z.cpmbackfill[bn++][0]) {}
                    if (z.cpmbackfill[bn] && z.cpmbackfill[bn][0] && z.cpmbackfill[bn][1].length > 0) {
                ...
                for (i = 0,
                a; i < c.length && (a = bsa.getads(c[i], 1)[0]); i++,
                o += '<li class="bsapt_' + a.id + ' ad' + i + ' ' + (i % 2 === 0 ? 'even' : 'odd') + '"><a ' + bsa.link(a.id, z.id, a.link) + ' target="_blank"><div class="bwr"><em class="bt">' + a.title + '<\/em><em class="bd">' + a.text + '<\/em><\/div><\/a><\/li>',
                t += a.id + ';') {}
                for (i = j; (typeof (bsa.callback) == 'function') && i < z.nads; i++,
                o += bsa.callback(z.id, i - 1, z.nads) || '') {}
                o += '<\/ul>';
                if (c.length > 0)
                    o += '<div class="bsa_idb"><span class="bsa_idl"><a href="https:\/\/www.buysellads.com\/buy\/detail\/' + z.siteid + '?utm_source=site_' + z.siteid + '&utm_medium=website&utm_campaign=textdesc&utm_content=zone_' + z.id + '" target="_blank">ads by <em>BSA<\/em><\/a><\/span><\/div><\/div><\/div>'
            } else if (tf == 4) {
...
(function() {
    ...
    window.onload = dequeue = function() {
        _bsap.exec();
        if (oldonload && dequeue != oldonload)
            oldonload()
    }
}
);
